...
TORCH_LIBRARY_EXPAND(CONCAT(TORCH_EXTENSION_NAME, _musa_ops), musa_ops) {
  ...
  musa_ops.def(
      "fp8_ds_mla_sparse_attention_grouped(Tensor q, Tensor cache, "
      "Tensor indices, Tensor? lengths, Tensor? attn_sink, "
      "Tensor? extra_cache, Tensor? extra_indices, Tensor? extra_lengths, "
      "Tensor! output, Tensor! lse, float softmax_scale) -> ()");
  musa_ops.impl("fp8_ds_mla_sparse_attention_grouped", torch::kMUSA,
