...
def _try_mhc_weighted_rms_norm_musa(
    ...
    if impl in {"", "0", "false", "off", "torch", "fallback"}:
        return None
    if impl not in {"auto", "1", "true", "on", "tilelang", "jit", "musa"}:
        raise ValueError(
            f"{_MHC_WEIGHTED_RMSNORM_IMPL_ENV} must be one of "
            "'auto', 'tilelang', 'torch', or 'off', got {impl!r}"
        )

    explicit_tilelang = impl in {"1", "true", "on", "tilelang", "jit", "musa"}
    ...
    if not supported:
        if explicit_tilelang:
            raise NotImplementedError(
                ...
                "MUSA bf16 x, contiguous bf16 norm_weight, and hidden_size "
                f"divisible by 64; got x_shape={tuple(x.shape)}, "
