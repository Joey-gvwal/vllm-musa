...
_MOE_SHAPE_INVENTORY_PATH_ENV = (
    "VLLM_MUSA_DEEPSEEK_V4_MOE_SHAPE_INVENTORY_PATH"
)
    ...
    for slot in range(ids_cpu.shape[1] if ids_cpu.dim() >= 2 else 0):
        ...
        if histogram_size > 0:
            slot_histograms.append(
                torch.bincount(
                    slot_ids[slot_valid], minlength=histogram_size
                ).tolist()
            )
        else:
            slot_histograms.append([])
    ...
    *,
    ...
    try:
        ...
        if not _MOE_SHAPE_INVENTORY_WARNED:
            logger.warning(
                "Failed to write DeepSeek-V4 MoE shape inventory: %s", exc
            )
