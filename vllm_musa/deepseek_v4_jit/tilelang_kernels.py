...
def mhc_pre_logits_kernel(
    ...
    @tilelang.jit(target="musa", pass_configs=_tilelang_musa_pass_configs(tilelang))
    def _mhc_pre_logits_kernel():
        num_tokens = T.dynamic("num_tokens")

        @T.prim_func
        def _kernel(
            residual: T.Tensor((num_tokens, 4, hidden_size), T.bfloat16),
            fn: T.Tensor((24, 4 * hidden_size), T.float32),
            mixes_raw: T.Tensor((num_tokens, 24), T.float32),
            sqsum: T.Tensor((num_tokens,), T.float32),
        ):
            with T.Kernel(num_tokens, 25, threads=threads) as (
                token_id,
                output_id,
            ):
                tx = T.get_thread_binding()
                ...
                warp_acc = T.alloc_shared((warps_per_cta,), T.float32)

                acc[0] = 0.0
                if output_id < 24:
                    for hc_idx in T.serial(0, 4):
                        for hidden_base in T.serial(0, hidden_size, threads):
                            hidden_idx = hidden_base + tx
                            if hidden_idx < hidden_size:
                                value = T.cast(
                                    residual[token_id, hc_idx, hidden_idx],
                                    T.float32,
                                )
                                flat_col = hc_idx * hidden_size + hidden_idx
                                acc[0] += value * fn[output_id, flat_col]
                else:
                    for hc_idx in T.serial(0, 4):
                        for hidden_base in T.serial(0, hidden_size, threads):
                            hidden_idx = hidden_base + tx
                ...
                acc[0] = _warp_reduce_sum(acc[0])
                if lane == 0:
                    warp_acc[warp] = acc[0]
                T.sync_threads()

                acc[0] = T.if_then_else(tx < warps_per_cta, warp_acc[tx], 0.0)
                if warp == 0:
                    acc[0] = _warp_reduce_sum(acc[0])
                    if lane == 0:
                        if output_id < 24:
                            mixes_raw[token_id, output_id] = acc[0]
                        else:
                            sqsum[token_id] = acc[0]

        return _kernel

    return _mhc_pre_logits_kernel()
