...
Currently **142 patches**. This branch includes the Qwen3.6 patches for common
...
page-aligned recurrent state backing for MUSA. The following QSA patches carry
the upstream prefill/decode split, sparse-GQA schedule, invalid-logit masking,
fast top-k warmup, FP8 indexer-cache support, and bounded prefill logits
workspace. The MUSA QSA path retains its opaque custom-op boundary and does not
enable the NVIDIA-only torch.compile removal.
The series contains
