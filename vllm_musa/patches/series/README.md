...
Currently **149 patches**. This branch includes the Qwen3.6 patches for common
...
DeepSeek-V4 MHC paths through MUSA providers. Five patches adapt the v0.28
Model Runner V2 rejection kernels to MUSA Triton scalar-predicate and
...
The Qwen3.8-Flash-Next patches backport its QSA Triton implementation and
adapt its FlashAttention KV layout, persistent top-k selection, and
page-aligned recurrent state backing for MUSA. Later QSA patches carry the
upstream prefill/decode split, sparse-GQA schedule, invalid-logit masking,
fast top-k warmup, FP8 indexer-cache support, bounded prefill logits
workspace, Qwen4Exp HC unit-injection fusion, PLE fused ops, and the stale
rotary-position guard. Additional MUSA adapters keep CAR+RMS on the MUSA
matcher, specialize Triton warmup scalars/strides, and import FlashAttention
helpers from the MUSA backend. The MUSA QSA path retains its opaque custom-op
boundary and does not enable the NVIDIA-only torch.compile removal. The series
is a contiguous `0001`–`0149` `git format-patch` stack; count the `.patch`
files rather than historical filenames.
The series contains
