...
_DSV4_TOKEN_BYTES = _DSV4_TOKEN_DATA_BYTES + _DSV4_TOKEN_SCALE_BYTES
...
def _flatten_mqa_k_cache(name: str, k_cache: torch.Tensor) -> torch.Tensor:
    ...
    if not k_cache.is_floating_point():
        raise RuntimeError(
            ...
            f"KV cache; {name} has dtype={k_cache.dtype}. "
...
def _gather_fp8_ds_mla_sparse_kv(
    ...
    flat_cache = _fp8_ds_mla_cache_physical_rows(cache)
...
def _fp8_ds_mla_cache_physical_rows(cache: torch.Tensor) -> torch.Tensor:
    """Expose packed fp8_ds_mla cache rows without copying alignment padding."""
    num_blocks = cache.shape[0]
    block_size = cache.shape[1]
    logical_row_bytes = block_size * _DSV4_TOKEN_BYTES
    row_stride = int(cache.stride(0))
    if row_stride < logical_row_bytes:
        raise RuntimeError(
            "Torch sparse FlashMLA fallback expected fp8_ds_mla cache row "
            f"stride >= logical row bytes, got stride={row_stride}, "
            f"logical={logical_row_bytes}, shape={cache.shape}."
        )
    if cache.stride(-1) != 1:
        raise RuntimeError(
            "Torch sparse FlashMLA fallback expected fp8_ds_mla cache rows "
            f"to have contiguous byte layout, got shape={cache.shape}, "
            f"stride={cache.stride()}."
        )
    return cache.as_strided((num_blocks, row_stride), (row_stride, 1))
...
# The backend availability checks only need to confirm that MUSA can provide a
# dense FlashMLA path or the torch sparse correctness provider below.
