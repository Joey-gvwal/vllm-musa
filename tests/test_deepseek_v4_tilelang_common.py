...
def test_pass_config_default_is_disabled(monkeypatch):
    ...
    monkeypatch.delenv(
        "VLLM_MUSA_DEEPSEEK_V4_TILELANG_COMPILE_PROFILE", raising=False
    )
...
def test_compile_profile_and_aggressive_switches(monkeypatch):
    ...
    monkeypatch.setenv(
        "VLLM_MUSA_DEEPSEEK_V4_TILELANG_AGGRESSIVE_PASS_CONFIG", "1"
    )
    monkeypatch.setenv(
        "VLLM_MUSA_DEEPSEEK_V4_TILELANG_DISABLE_INDEX_PROMOTION", "1"
    )
