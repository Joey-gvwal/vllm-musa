...
def test_flashmla_cache_pack_uses_one_warp_per_fp8_group():
    ...
    assert "template <bool kOptimizedCachePack>" in source
...
def test_flashmla_cache_pack_keeps_legacy_ab_switch():
    source = (ROOT / "csrc/musa/attention/deepseek_v4_cache_store.mu").read_text()

    assert "VLLM_MUSA_DEEPSEEK_V4_QNORM_ROPE_KV_PACK_IMPL" in source
    assert 'std::strcmp(value, "legacy") != 0' in source
    assert "deepseek_v4_qnorm_rope_kv_pack_fused_kernel<true>" in source
    assert "deepseek_v4_qnorm_rope_kv_pack_fused_kernel<false>" in source
